...
export default function TraceProjectCommandCenterPage(){
 ...
 async function saveActivity(){if(!selectedExecution||!activityText.trim()){setError(activityType==="correction.responded"?"Selecciona el trabajo y describe la respuesta a la corrección.":"Selecciona el trabajo y escribe la nota que deseas conservar en la bitácora.");return}setBusy(true);setError("");try{await req("/api/trace/v1/admin/activities",{method:"POST",body:JSON.stringify({executionId:selectedExecution,executionStageId:activityStageId||undefined,executionActivityId:activityExecutionActivityId||undefined,type:activityType,description:activityText.trim()})});setActivityOpen(false);setActivityText("");setActivityStageId("");setActivityExecutionActivityId("");setMessage(activityType==="correction.responded"?"Respuesta registrada en la bitácora.":"Nota añadida a la bitácora.");setTimelineRefresh(x=>x+1);await loadProject()}catch(e){setError(e.message)}finally{setBusy(false)}}
 ...
 function openRegister(type="comment.added",event=null){if(event?.execution_id)setSelectedExecution(event.execution_id);else setSelectedExecution(selectedExecution||work[0]?.id||"");setActivityStageId(event?.execution_stage_id||"");setActivityExecutionActivityId(event?.execution_activity_id||"");setActivityType(type);setActivityText(type==="correction.responded"?"":"");setActivityOpen(true)}
 function openStageRegister(stage){setSelectedExecution(selectedExecution||work[0]?.id||"");setActivityStageId(stage?.id||"");setActivityExecutionActivityId("");setActivityType("comment.added");setActivityText("");setActivityOpen(true)}
 ...
 return <TraceProjectShell view={view} onView={setView} projects={projects} projectId={projectId} onProjectChange={setProjectId} incidentCount={Number(summary?.metrics?.openIncidents||dashboard?.metrics?.openIncidents||0)} userLabel={user?.email||"Usuario"}>
  ...
  <TraceActivityModal open={activityOpen} work={work} executionId={selectedExecution} setExecutionId={setSelectedExecution} type={activityType} description={activityText} setDescription={setActivityText} busy={busy} onClose={()=>{setActivityOpen(false);setActivityStageId("");setActivityExecutionActivityId("")}} onSave={saveActivity}/>
