...
export default function TraceProjectCommandCenterPage(){
 ...
 const[activityOpen,setActivityOpen]=useState(false),[activityText,setActivityText]=useState(""),[activityType,setActivityType]=useState("progress.updated"),[selectedExecution,setSelectedExecution]=useState(""),[activityStageId,setActivityStageId]=useState(""),[busy,setBusy]=useState(false),[selectedIncident,setSelectedIncident]=useState(null),[selectedTimelineEvent,setSelectedTimelineEvent]=useState(null),[timelineRefresh,setTimelineRefresh]=useState(0);
 ...
 async function saveActivity(){if(!selectedExecution||!activityText.trim()){setError("Selecciona una actividad del proyecto y describe lo ocurrido.");return}setBusy(true);setError("");try{await req("/api/trace/v1/admin/activities",{method:"POST",body:JSON.stringify({executionId:selectedExecution,executionStageId:activityStageId||undefined,type:activityType,description:activityText.trim()})});setActivityOpen(false);setActivityText("");setActivityStageId("");setMessage("Actividad registrada en la bitácora.");setTimelineRefresh(x=>x+1);await loadProject()}catch(e){setError(e.message)}finally{setBusy(false)}}
 ...
 function openStageRegister(stage){setSelectedExecution(selectedExecution||work[0]?.id||"");setActivityStageId(stage?.id||"");setActivityType("progress.updated");setActivityText(stage?.name?`${stage.name}: `:"");setActivityOpen(true)}
 ...
 return <TraceProjectShell view={view} onView={setView} projects={projects} projectId={projectId} onProjectChange={setProjectId} incidentCount={Number(summary?.metrics?.openIncidents||dashboard?.metrics?.openIncidents||0)} userLabel={user?.email||"Usuario"}>
  ...
  <TraceActivityModal open={activityOpen} work={work} executionId={selectedExecution} setExecutionId={setSelectedExecution} type={activityType} setType={setActivityType} description={activityText} setDescription={setActivityText} busy={busy} onClose={()=>{setActivityOpen(false);setActivityStageId("")}} onSave={saveActivity}/>
