...
export default function TraceOperationalPage() {
  ...
  function changeAccess(){localStorage.removeItem("trace_operational_token");setToken("");setTasks([]);setSelected(null);setError("")}
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-900">
    <div className="mx-auto max-w-md min-h-screen bg-white shadow-sm">
      <header className="sticky top-0 z-10 border-b bg-white px-4 py-4">
        ...
        <div className="flex items-center justify-between gap-3"><div className="text-xl font-bold">Portal operacional</div>{activeToken?<button className="text-xs text-slate-500" onClick={changeAccess}>Cambiar acceso</button>:null}</div>
      </header>
      <main className="p-4 space-y-4">
        {!activeToken ? <section className="rounded-2xl border p-4 space-y-3">
        ...
        {!selected && activeToken ? <section className="space-y-3">
          ...
          {loading?<div className="py-10 text-center text-slate-400">Cargando…</div>:tasks.length===0?<div className="rounded-2xl border border-dashed p-6 text-center"><div className="font-semibold">No tienes tareas activas</div><div className="mt-1 text-sm text-slate-500">Cuando se te asigne una etapa, una corrección o una revisión, aparecerá aquí.</div></div>:tasks.map(t=><button key={t.id} onClick={()=>openTask(t.id)} className="w-full rounded-2xl border p-4 text-left shadow-sm">
            ...
            {t.permissions?.correctionRequired?<div className="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-xs font-medium text-amber-800">Requiere corrección asignada</div>:null}
            {t.permissions?.awaitingApproval?<div className="mt-3 rounded-lg bg-blue-50 px-3 py-2 text-xs font-medium text-blue-700">En espera de aprobación</div>:null}
          </button>)}
        </section>:null}
        {selected?<section className="space-y-4">
          <button className="text-sm text-blue-600" onClick={()=>setSelected(null)}>← Mis tareas</button>
          <div className="rounded-2xl border p-4"><div className="text-xs text-slate-500">{selected.executionCode}</div><h1 className="mt-1 text-xl font-bold">{selected.stage?.name}</h1><p className="mt-2 text-sm text-slate-600">{selected.stage?.instructions}</p><div className="mt-4 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-slate-100 px-2 py-1">{selected.stage?.status}</span>{selected.stage?.requiresEvidence?<span className="rounded-full bg-amber-50 px-2 py-1 text-amber-700">Requiere evidencia</span>:null}</div></div>
          {selected.stage?.status==="correction_required"||selected.stage?.status==="rejected"?<div className="rounded-2xl bg-amber-50 p-4 text-sm text-amber-900"><div className="font-semibold">Esta etapa requiere corrección</div><div className="mt-1">La tarea permanece visible hasta que el ciclo de corrección sea resuelto.</div></div>:null}
          {selected.stage?.status==="pending_approval"?<div className="rounded-2xl bg-blue-50 p-4 text-sm text-blue-800"><div className="font-semibold">En espera de aprobación</div><div className="mt-1">La etapa fue enviada y permanece visible mientras el supervisor decide.</div></div>:null}
