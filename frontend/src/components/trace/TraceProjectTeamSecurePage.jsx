...
export default function TraceProjectTeamSecurePage({projectId}){
 ...
 async function createInvite(body){setBusy(true);setError('');setOpen(false);try{const d=await api(`/api/trace/v1/admin/projects/${encodeURIComponent(projectId)}/team/invitations`,{method:'POST',body:JSON.stringify(body)});setResult(d.data);await load()}catch(e){setError(e.message)}finally{setBusy(false)}}
 ...
 return <div className="trace-secure-team relative"><style>{`.trace-secure-team > .team-base > div > header > div.relative > button:first-child{display:none!important}`}</style>
  <div className="mb-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3"><div><div className="text-xs font-black text-blue-900">Acceso de miembros por invitación segura</div><div className="mt-1 text-[11px] text-blue-700">Enlace de un solo uso + código temporal. El correo automático se conectará después.</div></div><button onClick={()=>{setError('');setOpen(true)}} className="rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-black text-white">＋ Invitar miembro</button></div>
  {error&&<div className="mb-3 flex items-center justify-between gap-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-xs font-semibold text-red-700"><span>{error}</span><div className="flex shrink-0 items-center gap-2"><button onClick={()=>setError('')} className="rounded-lg border border-red-300 px-3 py-1.5 text-[10px] font-black">Aceptar</button><button onClick={()=>setError('')} aria-label="Cerrar aviso" className="grid h-7 w-7 place-items-center rounded-lg hover:bg-red-100">×</button></div></div>}
...
function InviteResult({result,projectName,onClose}){
 ...
 const project=result.projectName||projectName||null;
...
function Modal({title,onClose,children}){return <div className="fixed inset-0 z-[170] grid place-items-center bg-slate-950/45 p-4"><div className="max-h-[92vh] w-full max-w-md overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl"><div className="mb-5 flex items-center justify-between"><h2 className="text-xl font-black">{title}</h2><button onClick={onClose} className="text-xl text-slate-400">×</button></div>{children}</div></div>}
