...
export default function TraceStandaloneShell({ view, onView, onNewControl, onRegister, children }) {
  ...
  return (
    <div className="min-h-screen bg-[#f5f7fb] text-slate-900">
      <aside className={`fixed inset-y-0 left-0 z-40 flex flex-col bg-[#071226] text-white shadow-2xl transition-all ${collapsed ? "w-[76px]" : "w-[222px]"}`}>
      ...
      <div className={`min-h-screen transition-all ${collapsed ? "pl-[76px]" : "pl-[222px]"}`}>
        <header className="sticky top-0 z-30 flex h-[70px] items-center gap-3 border-b border-slate-200 bg-white/95 px-5 backdrop-blur xl:px-7">
          ...
          <select className="hidden rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-semibold text-slate-700 md:block"><option>Todos los proyectos</option><option>Proyecto Vista Real</option></select>
          <button className="relative grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:border-blue-300">♢<span className="absolute -right-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full bg-red-500 px-1 text-[9px] font-bold text-white">2</span></button>
