...
export async function handleTraceV1OnboardingExecutionApi(request,env){
 ...
 const stages=await c.db.prepare(`SELECT id,stage_order,responsible_role FROM trace_stages WHERE process_version_id=? ORDER BY stage_order`).bind(project.current_version_id).all();
 ...
 const definitions=await loadDefinedActivities(c.db,c.tenantId,project.current_version_id);
 ...
 const stageInstances=(stages.results||[]).map((s,i)=>({stageId:s.id,stageOrder:Number(s.stage_order),executionStageId:crypto.randomUUID(),assignedTo:i===0?c.user.id:null,responsibleRole:s.responsible_role}));
 ...
 for(const item of stageInstances){statements.push(c.db.prepare(`INSERT INTO trace_execution_stages (id,execution_id,stage_id,stage_order,status,assigned_to,response_json,validation_json,created_at,updated_at) VALUES (?,?,?,?,?,?, '{}','{}',datetime('now'),datetime('now'))`).bind(item.executionStageId,executionId,item.stageId,item.stageOrder,item.stageOrder===stageInstances[0].stageOrder?'available':'pending',item.assignedTo))}
 const materialized=buildExecutionActivityStatements(c.db,{tenantId:c.tenantId,executionId,definitions,executionStages:stageInstances,roleAssignments:{}});statements.push(...materialized.statements);
 statements.push(c.db.prepare(`INSERT INTO trace_events (id,tenant_id,execution_id,execution_stage_id,asset_id,event_type,event_source,actor_user_id,actor_role,description,payload_json,occurred_at,received_at) VALUES (?,?,?,?,?,'execution.created','admin',?,?,?, ?,datetime('now'),datetime('now'))`).bind(crypto.randomUUID(),c.tenantId,executionId,stageInstances[0].executionStageId,projectId,c.user.id,'manager',`Trace fue activado para ${project.name}.`,JSON.stringify({activitiesMaterialized:materialized.activityRows.length})));
 await c.db.batch(statements);
 return json({ok:true,data:{executionId,executionCode,status:"assigned",reused:false,activitiesMaterialized:materialized.activityRows.length}},201);
}
