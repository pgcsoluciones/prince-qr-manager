...
async function createInvite(request,env,projectId){
 ...
 if(!inviteEmail||!inviteEmail.includes('@'))return json({ok:false,error:'valid_email_required'},422);if(b.phone&&!requestedPhone)return json({ok:false,error:'valid_phone_required',message:'Indica un teléfono válido. Para República Dominicana puedes escribir 809, 829 o 849 sin +1; KAWVO lo completa automáticamente.'},422);
 ...
 const storedPhone=phone(orgMember.phone_e164),phoneE164=storedPhone||requestedPhone;if(!phoneE164)return json({ok:false,error:'organization_member_phone_required',message:'Este miembro no tiene teléfono registrado. Actualiza sus datos antes de generar la invitación.'},422);
 if(requestedPhone&&storedPhone&&requestedPhone!==storedPhone)return json({ok:false,error:'phone_mismatch',message:'El teléfono indicado no coincide con el registrado para este miembro.'},422);
 ...
 return json({ok:true,message:'Invitación creada correctamente.',data:{mode:'invited',invitationId,email:inviteEmail,phone:phoneE164,projectRole,inviterName:inviterName(c.user),companyName:companyFromProject(project),projectName:project.name,...claim}},201);
}
