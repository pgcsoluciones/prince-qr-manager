...
async function supervisorDashboard(request,env){
 const a=await auth(request,env); if(!a.ok)return a.response; const {session,db}=a;
 const allowedRole=await db.prepare(`SELECT COUNT(*) n FROM trace_execution_participants WHERE tenant_id=? AND user_id=? AND status='active' AND participation_role IN ('process_owner','department_lead','reviewer','approver') AND (? IS NULL OR execution_id=?)`).bind(session.tenant_id,session.user_id,session.execution_id||null,session.execution_id||null).first();
 if(Number(allowedRole?.n||0)===0)return json({ok:false,error:"forbidden",message:"Esta sesión operacional no tiene funciones de supervisión en su alcance actual."},403);
