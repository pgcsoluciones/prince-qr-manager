...
async function createReport(c,projectId,request){
 ...
 const id=uuid(),title=text(body.title,180)||`${typeLabel(type)} · ${new Date().toLocaleDateString("es-DO",{year:"numeric",month:"long",day:"numeric"})}`,description=text(body.description,800);
 await c.db.prepare(`INSERT INTO trace_reports (id,tenant_id,project_id,execution_id,report_type,title,description,scope_type,scope_id,period_from,period_to,status,definition_json,snapshot_json,generated_by,generated_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,'ready',?,?,?,datetime('now'),datetime('now'),datetime('now'))`).bind(id,c.tenantId,projectId,definition.scope.type||"project"=== "execution"?definition.scope.id:null,type,title,description,definition.scope.type||"project",definition.scope.id||null,definition.period.from||null,definition.period.to||null,JSON.stringify(definition),JSON.stringify(snapshot),c.user.id).run();
